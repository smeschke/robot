...
R5D2 is a human-scale, differential-drive utility robot designed to move comfortably through real-world, ADA-compliant spaces at walking speed. It uses two large independently powered wheels and a rear caster—similar in spirit to a robotic vacuum.
...
The robot uses two 550-type brushed DC motors with 100:1 reduction gearboxes driving 10-inch pneumatic wheels. It’s powered by a 12-volt, 7 amp-hour sealed lead-acid battery. Motor control is handled by BTS7960 H-bridge motor drivers. For sensing, the robot uses a GY-521 IMU.

A single main power switch sits between the battery and the system. In the minimum configuration, the switch can be wired to control both the motor drivers and a DC-DC buck converter for the ESP32.
